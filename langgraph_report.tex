...
\begin{tikzpicture}[font=\small]
    % Top level
    \node[datanode] (start) at (0,0) {START};
    \node[hybridnode, minimum width=4.4cm] (classify) at (0,-1.8) {classify\\keyword fast-path + LLM fallback};

    % Main branches
    \node[datanode] (fetchdata) at (-4.8,-4.3) {fetch\_data};
    \node[llmnode] (answerdata) at (-2.0,-4.3) {answer\_data};

    \node[llmnode] (answerguide) at (4.8,-4.3) {answer\_guide};

    \node[datanode] (fetchrisk) at (-1.4,-6.2) {fetch\_risk};
    \node[llmnode] (answerrisk) at (1.4,-6.2) {answer\_risk};

    \node[llmnode] (answernoloc) at (-4.2,-8.1) {answer\_risk\_no\_loc};
    \node[llmnode] (answersmall) at (4.2,-8.1) {answer\_smalltalk};

    % Merge / end
    \node[mergepoint] (merge) at (0,-10.1) {};
    \node[datanode] (end) at (0,-11.3) {END};

    % Entry
    \draw[line] (start) -- (classify);

    % Outgoing routes from classify
    \draw[line] (classify.south west) to[out=220,in=90] node[pos=0.45, above, font=\scriptsize] {data} (fetchdata.north);
    \draw[line] (classify.south east) to[out=-40,in=90] node[pos=0.45, above, font=\scriptsize] {guide} (answerguide.north);
    \draw[line] (classify.south) -- node[right, font=\scriptsize] {risk} (fetchrisk.north);
    \draw[line] (classify.south west) to[out=235,in=90] node[pos=0.55, left, font=\scriptsize] {risk\_no\_loc} (answernoloc.north);
    \draw[line] (classify.south east) to[out=-55,in=90] node[pos=0.55, right, font=\scriptsize] {smalltalk} (answersmall.north);

    % Internal chained paths
    \draw[line] (fetchdata) -- (answerdata);
    \draw[line] (fetchrisk) -- (answerrisk);

    % Success paths to END
    \draw[successline] (answerdata.south) to[out=-90,in=160] (merge);
    \draw[successline] (answerguide.south) to[out=-90,in=20] (merge);
    \draw[successline] (answerrisk.south) -- (merge);
    \draw[successline] (answernoloc.east) to[out=0,in=180] (merge);
    \draw[successline] (answersmall.west) to[out=180,in=0] (merge);
    \draw[successline] (merge) -- (end);
\end{tikzpicture}
